...
var (
  ...
  ## In case of mutual authentication client.crt,client.key and ca.crt files must be 
  ## provided
  ssl_cert = "PKI/client.pem"
  mutual_auth = true
...
func (m *OpenConfigTelemetry) Start(acc telegraf.Accumulator) error {
    ...
    if m.SSLCert != "" {
        ...
        // If mutual authentication is required , use client.crt,clien.key and ca.crt files
        if m.MutualAuth {
            cert, err := tls.LoadX509KeyPair(m.ClientCrt,m.ClientKey)
            //cert, err := tls.LoadX509KeyPair(fmt.Sprintf("%s/mx1_re.crt", m.SSLCert), fmt.Sprintf("%s/mx1_re.key", m.SSLCert))
            if err != nil {
                return fmt.Errorf("E! Could not load certFile: %v", err)
            }
            certPool := x509.NewCertPool()
            ca, err := ioutil.ReadFile(m.CaCrt)
            if err != nil {
                return fmt.Errorf("E! Could not read certificate: %v", err)
            }
            if ok := certPool.AppendCertsFromPEM(ca); !ok {
                return fmt.Errorf("E! Failed to append client certs")
            }
    ...
    //log.Printf("Credentials %s:", transportCredentials)
